...
const TripsView = ({ onSelectTrip }) => {
    const { myTrips } = useApp();

    return (
        <div id="view-explore" className="view-section active">
            <div style={{ padding: '20px', fontWeight: 700, fontSize: '1.7rem' }}>My Trips</div>
            <div className="trip-list" id="tripsListContainer">
                {myTrips.map((trip, index) => (
                    <div className="trip-card" key={index} onClick={() => onSelectTrip && onSelectTrip(trip)} style={{ cursor: 'pointer' }}>
